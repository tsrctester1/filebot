...
def find_relevant_info(user_prompt, max_token_length=3900):
    ...
    user_prompt = f"{prepend_prompt}. Based on the following summaries```{file_summaries}``` which file or files based on the summaries should we open to see if it has any info regarding. List the most promising files first: ```{user_prompt}```"
